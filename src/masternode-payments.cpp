...
*
*   Why is this needed?
*   - In Waggox some blocks are superblocks, which output much higher amounts of coins
...
{
    if(fLiteMode) return; // disable all Waggox specific functionality
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew) const
{
    ...
    LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f WAGGOX\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
