...
const char * const BITCOIN_CONF_FILENAME = "waggox.conf";
const char * const BITCOIN_PID_FILENAME = "waggoxd.pid";
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\WaggoxCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\WaggoxCore
    // Mac: ~/Library/Application Support/WaggoxCore
    // Unix: ~/.waggoxcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "WaggoxCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/WaggoxCore";
#else
    // Unix
    return pathRet / ".waggoxcore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty waggox.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override waggox.conf
